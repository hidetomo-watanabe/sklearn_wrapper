{
 "cells": [
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "logger.info('### PLOT X TRAIN HISTOGRAM WITH PRED GROUP')\n",
    "visualizer_obj.plot_x_train_histogram_with_pred_group(\n",
    "    raw_data['train_df'].sample(frac=1, replace=False, random_state=42),\n",
    "    raw_data['pred_df'].sample(frac=1, replace=False, random_state=42))"
   ]
  },
  {
   ...
   "source": [
    ...
    "visualizer_obj.plot_x_train_test_histogram(\n",
    "    raw_data['train_df'].sample(frac=1, replace=False, random_state=42),\n",
    "    raw_data['test_df'].sample(frac=1, replace=False, random_state=42))"
   ]
  },
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "logger.info('### CHECK CONFUSION MATRIX')\n",
    "from sklearn.metrics import confusion_matrix\n",
    "confusion_matrix(train_data['Y_train'], pred_data['Y_train_pred'].reshape(-1, 1))"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "visualizer_obj.plot_confusion_matrix(\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "    train_data['Y_train'], pred_data['Y_train_pred_proba'])"
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [],
   "source": [
    "logger.info('### PLOT Y TRAIN TEST PRED HISTOGRAM')\n",
    "visualizer_obj.plot_y_train_test_pred_histogram(\n",
    "    pred_data['Y_train_pred'].reshape(-1, 1), pred_data['Y_pred'].reshape(-1, 1))"
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [],
   "source": [
    "logger.info('### PLOT Y TRAIN TEST PRED PROBA HISTOGRAM')\n",
    "visualizer_obj.plot_y_train_test_pred_proba_histogram(\n",
    "    pred_data['Y_train_pred_proba'].reshape(-1, 1), pred_data['Y_pred_proba'].reshape(-1, 1))"
   ]
  },
  {
   "cell_type": "code",
